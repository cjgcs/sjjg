...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=0;i<9;i++) {
			int x = kb.nextInt();
			L.insert(i, new RecordNode(x,null));
		}
		L.mergeSort();
	}
...
//      System.out.println("归并排序");
      int s = 2;                      //s为已排序的子序列长度，初值为1
      int n = this.curlen;
      RecordNode[] temp = new RecordNode[n];  //定义长度为n的辅助数组temp
      RecordNode[] tp = new RecordNode[n];
      mergepass(r, temp, 1, n);  //一趟归并，将r数组中各子序列归并到temp中
      display();
      while (s < n) {
          tp=temp;
          temp=r;
          r=tp;
          mergepass(r, temp, s, n);  //将temp数组中各子序列再归并到r中
          display();
          s *= 2;
      }
      s=1;
